...
public abstract class AbstractTree<E> implements Tree<E> {
  ...
  public Iterable<Position<E>> breadthfirst() {
    ...
    if (!isEmpty()) {
      ...
      ((LinkedQueue) fringe).enqueue(root());                 // start with the root
      while (!fringe.isEmpty()) {
        Position<E> p = (Position<E>) ((LinkedQueue) fringe).dequeue();     // remove from front of the queue
        snapshot.add(p);                      // report this position
        for (Position<E> c : children(p))
          ((LinkedQueue) fringe).enqueue(c);                  // add children to back of queue
      }
    }
    return snapshot;
  }
